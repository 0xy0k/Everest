...
import { EverestResultPromise } from '../types/EverestResult';
...
async function _callNxtp(
  ...
  try {
    ...
    if (!isSupported) {
      return new EverestResultError('Asset is not supported by Connext!');
    }
...
async function depositAndBorrow(
  ...
  if (vault instanceof LendingVault) {
    return new EverestResultError('Wrong vault type');
  }
