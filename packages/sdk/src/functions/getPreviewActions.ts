...
import { EverestResultSuccess } from '../entities';
...
function payback(
  ...
  if (op === OperationType.ONE_CHAIN) {
    // everything happens on the same chain
    return new EverestResultSuccess([_payback(vault, amountIn, account, account)]);
  } else {
...
function paybackAndWithdraw(
  ...
  if (op === OperationType.ONE_CHAIN) {
    ...
    return new EverestResultSuccess([xTransfer]);
  } else {
    return new EverestResultSuccess([]);
  }
}
