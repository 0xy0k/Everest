...
import { EverestResult, EverestResultPromise, PreviewParams } from './types';
...
export class Previews {
  getOperationType(params: PreviewParams): EverestResult<OperationType> {
    ...
    if (name === PreviewName.DEPOSIT || name === PreviewName.PAYBACK) {
      if (params.tokenIn.chainId !== srcChainId) {
        return new EverestResultError('Unsupported DEPOSIT or PAYBACK operation!');
      }
      if (srcChainId === vault.chainId) {
        return new EverestResultSuccess(OperationType.ONE_CHAIN);
      } else {
        return new EverestResultSuccess(OperationType.TWO_CHAIN_VAULT_ON_DEST);
      }
      ...
      if (
        ...
        return new EverestResultSuccess(OperationType.TWO_CHAIN_VAULT_ON_SRC);
      } else if (
        srcChainId !== vault.chainId &&
        params.tokenOut.chainId === vault.chainId
      ) {
        ...
        return new EverestResultSuccess(OperationType.THREE_CHAIN);
      }
    } else {
  ...
  async get(params: PreviewParams): EverestResultPromise<PreviewResult> {
  ...
  async deposit(
    ...
    account: Address,
    slippage?: number
  ): EverestResultPromise<PreviewResult> {
  ...
  async withdraw(
    ...
    if (srcChainId == vault.chainId && tokenOut.chainId === vault.chainId) {
      ...
      return new EverestResultError('3-chain transfers are not enabled yet!');
    }
  ...
  async inflowRoutingDetails(
    ...
    amountIn: BigNumber,
    tokenIn: Token
  ): EverestResultPromise<MetaRoutingResult> {
  ...
  ): EverestResult<XTransferWithCallParams> {
    const destDomain = CHAIN[destChainId].connextDomain;
    if (!destDomain) {
      return new EverestResultError(
        `Chain ${destChainId} is not available on Connext!`
