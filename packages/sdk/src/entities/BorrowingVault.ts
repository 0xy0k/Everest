...
export class BorrowingVault {
   ...
   */
  async getBorrowProviderStats(): EverestResultPromise<AprResult[]> {
   ...
   * provider is not available at DefiLlama, an empty array is returned.
   */
  async getSupplyProviderStats(): EverestResultPromise<AprResult[]> {
   ...
   */
  async signPermitFor(actionParams: RouterActionParams[]): EverestResultPromise<{
    digest: string;
    ...
    if (!permitParams) return new EverestResultError('No permit action to sign!');
    ...
    return new EverestResultSuccess({ digest, domain, types, value });
  }

  private async _getProvidersStatsFor(
    token: Token
  ): EverestResultPromise<AprResult[]> {
    ...
    if (!this.allProviders)
      return new EverestResultError('Lending Providers are not preLoaded!');
