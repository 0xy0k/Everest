...
contract LiquidationManagerPolygonForkingTest is ForkingSetup, Routines {
  ...
  function mock_getPriceOf(address asset1, address asset2, uint256 price) internal {
    vm.mockCall(
      address(oracle),
      abi.encodeWithSelector(IEverestOracle.getPriceOf.selector, asset1, asset2, 18),
      abi.encode(price)
    );
  }
    ...
    internal
  {
    ...
    vm.mockCall(
      address(oracle),
      abi.encodeWithSelector(IEverestOracle.getPriceOf.selector, asset1, asset2, decimalsAsset1),
    ...
    vm.mockCall(
      address(oracle),
      abi.encodeWithSelector(IEverestOracle.getPriceOf.selector, asset2, asset1, decimalsAsset2),
