...
contract ConnextRouter is BaseRouter, IXReceiver {
  ...
  ////////////////////////////////////*/
    ...
    ) external onlyConnextReceiver returns (bytes memory) {
        (Action[] memory actions, bytes[] memory args) = abi.decode(
            callData,
            (Action[], bytes[])
        );

        Snapshot memory tokenToCheck_ = Snapshot(
            asset,
    ...
    function _accountForSlippage(
        ...
        Action action,
        bytes memory args
    ) private pure returns (bytes memory newArgs, uint256 beforeSlipped) {
        uint256 prevAmount;
        (newArgs, prevAmount) = _replaceAmountArgInAction(
            action,
            args,
            receivedAmount
        );
    ...
    function _getBeneficiaryFromCalldata(
        ...
        bytes[] memory args
    ) internal view override returns (address beneficiary_) {
        if (actions[0] == Action.Deposit || actions[0] == Action.Payback) {
            ...
            (, , , , Action[] memory newActions, bytes[] memory newArgs) = abi
                .decode(
                    args[0],
                    (IFlasher, address, uint256, address, Action[], bytes[])
                );
            ...
            (Action[] memory actions_, bytes[] memory args_) = abi.decode(
                callData,
                (Action[], bytes[])
            );
