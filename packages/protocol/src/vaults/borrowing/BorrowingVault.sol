  ...
  */
  ...
  constructor(address asset_, address debtAsset_, address oracle_, address chief_)
    BaseVault(
      asset_,
      chief_,
      // name_, ex: X-Everest Dai Stablecoin Vault Shares
      string(abi.encodePacked("X-Everest ", IERC20Metadata(asset_).name(), " Vault Shares")),
      // symbol_, ex: xfDAI
  ...
  function setOracle(IEverestOracle newOracle) external {
