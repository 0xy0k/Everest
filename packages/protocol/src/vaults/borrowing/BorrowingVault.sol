...
contract BorrowingVault is BaseVault {
  ...
  ////////////////////*/
     ...
     * @param oracle_ of {EverestOracle} implementation
  ...
  ///////////////////////////////*/
    ...
    function _beforeTokenTransfer(
         ...
         */
        if (
            from != address(0) && to != address(0) && amount > maxRedeem(from)
        ) {
    ...
    function burnDebt(
        uint256 shares,
        address owner
    ) public override returns (uint256 debt) {
  ...
  ///////////////////////*/
     ...
     * Requirements:
     * - Must read price from {EverestOracle}.
     *
    ...
    function _computeMaxBorrow(
        ...
        uint256 baseUserMaxBorrow = assets.mulDiv(
            maxLtv * price,
            10 ** decimals() * PRECISION_CONSTANT
        );
    ...
    function _computeFreeAssets(
        address owner
    ) internal view returns (uint256 freeAssets) {
        ...
        if (debtShares == 0) {
            ...
            uint256 lockedAssets = debt.mulDiv(
                price * PRECISION_CONSTANT,
    ...
    function _paybackInternal(
        ...
        address owner,
        address caller
    ) internal returns (uint256 debt_, uint256 shares_) {
        uint256 remainder;
    ...
    function _paybackChecks(
        ...
        uint256 debt,
        uint256 shares
    ) private view returns (uint256 debt_, uint256 shares_, uint256 remainder) {
  ...
  ////////////////////*/
        ...
        returns (uint256 gainedShares)
    {
        ...
        uint256 debtToCover = Math.mulDiv(
            debt,
            liqCloseFactor_,
            PRECISION_CONSTANT
        );
        ...
        uint256 price = oracle.getPriceOf(debtAsset(), asset(), _debtDecimals);
        uint256 discountedPrice = Math.mulDiv(
            price,
            LIQUIDATION_PENALTY,
            PRECISION_CONSTANT
        );
  ...
  /////////////////////////*/
    ...
    /**
     * @notice Sets `newOracle` address as the {EverestOracle} for this vault.
     *
     ...
     */
    function setOracle(IEverestOracle newOracle) external onlyTimelock {
