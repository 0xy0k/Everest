...
if [ "$UTILS_OLD_VER" == "true" ] ; then
    ...
    TOOLS_VERSION="v0.0.8.0" && cd /tmp && rm -fv ./utils.sh && \
      wget "https://github.com/TsukiCore/tools/releases/download/$TOOLS_VERSION/tsuki-utils.sh" -O ./utils.sh && \
      ...
      chmod -v 555 ./utils.sh && ./utils.sh utilsSetup ./utils.sh "/var/tsukiglob" && . /etc/profile
else
...
if ($(isNullOrEmpty "$BUF_VER")) || [ "$TSUKI_PROTO_DEP_VER" != "$EXPECTED_PROTO_DEP_VER" ] ; then
    GO111MODULE=on 
    ...
    setGlobEnv TSUKI_PROTO_DEP_VER "$EXPECTED_PROTO_DEP_VER"
fi
...
cp -r github.com/TsukiCore/tsuki/* ./
rm -rf github.com
