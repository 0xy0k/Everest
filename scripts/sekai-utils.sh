...
function enableCustody() {
  ...
  tsukid tx custody create $MODE $PASSWORD $LIMITS $WHITELIST --from=$FROM --keyring-backend=test --chain-id=$NETWORK_NAME --fees="${FEE_AMOUNT}${FEE_DENOM}" --output=json --yes --home=$TSUKID_HOME --okey=$OKEY --nkey=$NKEY | txAwait 180
}
# enableCustody
...
function addCustodians() {
  ...
  tsukid tx custody custodians add "$ADDRESSES" --from=$FROM --keyring-backend=test --chain-id=$NETWORK_NAME --fees="${FEE_AMOUNT}${FEE_DENOM}" --output=json --yes --home=$TSUKID_HOME --okey=$OKEY --nkey=$NKEY | txAwait 180
}
