...
function tsukiUtilsVersion() {
    tsukiUtilsSetup "version" 2> /dev/null || tsuki-utils tsukiUtilsSetup "version"
}

# this is default installation script for tsukid utils
# ./tsuki-utils.sh tsukiUtilsSetup
function tsukiUtilsSetup() {
    local TSUKI_UTILS_VERSION="v0.0.1.3"
    if [ "$1" == "version" ] ; then
        echo "$TSUKI_UTILS_VERSION"
        return 0
    else
        local UTILS_SOURCE=$(realpath "$0")
        local VERSION=$($UTILS_SOURCE tsukiUtilsVersion || echo '')
        local UTILS_DESTINATION="/usr/local/bin/tsuki-utils.sh"

        if [ "$VERSION" != "$TSUKI_UTILS_VERSION" ] ; then
            bash-utils echoErr "ERROR: Self check version mismatch, expected '$TSUKI_UTILS_VERSION', but got '$VERSION'"
            return 1
            ...
            cp -fv "$UTILS_SOURCE" "/usr/local/bin/tsuki-utils"
            chmod -v 555 $UTILS_DESTINATION "/usr/local/bin/tsuki-utils"
            ...
            bash-utils setGlobEnv TSUKI_TOOLS_SRC "$UTILS_DESTINATION"
            ...
            if [[ $AUTOLOAD_SET -lt 0 ]] ; then
                echo "source $UTILS_DESTINATION || echo \"ERROR: Failed to load tsukid utils from '$UTILS_DESTINATION'\"" >> /etc/profile
            fi

            bash-utils loadGlobEnvs
            bash-utils echoInfo "INFO: SUCCESS, Installed tsuki-utils $(tsuki-utils tsukiUtilsVersion)"
        fi
    fi
}

function txQuery() {
    (! $(isTxHash "$1")) && echoErr "ERROR: Infalid Transaction Hash '$1'" && tsukid query tx "$1" --output=json --home=$TSUKID_HOME | jq || echoErr "ERROR: Transaction '$1' was NOT found or failed"
}
...
function txAwait() {
    ...
    while : ; do
        ...
        local OUT=$(tsukid query tx $TXHASH --output=json --home=$TSUKID_HOME 2> /dev/null | jsonParse "" 2> /dev/null || echo -n "")
        if [ ! -z "$OUT" ] ; then
...
function showAddress() {
    ($(isTsukiAddress "$1")) && echo "$1" || echo $(tsukid keys show "$1" --keyring-backend=test --output=json --home=$TSUKID_HOME 2> /dev/null | jsonParse "address" 2> /dev/null || echo -n "")
}

function showKeys() {
    tsukid keys list --keyring-backend=test --output=json --home=$TSUKID_HOME | jq
}

# showPermissions validator
function showPermissions() {
    local ADDRESS=$(showAddress $1)
    echo $(tsukid query customgov permissions "$ADDRESS" --output=json --home=$TSUKID_HOME 2> /dev/null | jsonParse 2> /dev/null || echo -n "") && echo -n ""
}
...
function lastProposal() {
    local BOTTOM_PROPOSALS=$(tsukid query customgov proposals --limit=1 --reverse --output=json --home=$TSUKID_HOME | jq -cr '.proposals | last | .proposal_id' 2> /dev/null || echo "")
    local TOP_PROPOSALS=$(tsukid query customgov proposals --limit=1 --output=json --home=$TSUKID_HOME | jq -cr '.proposals | last | .proposal_id' 2> /dev/null || echo "")
...
function voteProposal() {
    ...
    tsukid tx customgov proposal vote $PROPOSAL $VOTE --from=$ACCOUNT --chain-id=$NETWORK_NAME --keyring-backend=test  --fees=100ukex --yes --log_format=json --broadcast-mode=async --output=json --home=$TSUKID_HOME | txAwait
}
...
function showNetworkProperties() {
    local NETWORK_PROPERTIES=$(tsukid query customgov network-properties --output=json --home=$TSUKID_HOME 2> /dev/null || echo "" | jq -rc 2> /dev/null || echo "")
...
function showVotes() {
    tsukid query customgov votes "$1" --output=json --home=$TSUKID_HOME | jsonParse
}

# showProposal $(lastProposal) 
function showProposal() {
    tsukid query customgov proposal "$1" --output json --home=$TSUKID_HOME | jsonParse
}

function showProposals() {
    tsukid query customgov proposals --limit=999999999 --output=json --home=$TSUKID_HOME | jsonParse
}
...
function claimValidatorSeat() {
    ...
    tsukid tx customstaking claim-validator-seat --from "$ACCOUNT" --keyring-backend=test --home=$TSUKID_HOME --moniker="$MONIKER" --chain-id=$NETWORK_NAME --broadcast-mode=async --fees=100ukex --yes --output=json | txAwait $TIMEOUT
}
...
function showBalance() {
    ...
    local DENOM="$2"
    local RESULT=""
    (! $(isNullOrEmpty $ADDR)) && RESULT=$(tsukid query bank balances "$ADDR" --output=json --home=$TSUKID_HOME 2> /dev/null | jsonParse 2> /dev/null || echo -n "")
...
# e.g. sendTokens faucet tsukiXXX...XXX 1000 ukex 100 ukex
function sendTokens() {
    local SOURCE=$1
    ...
    tsukid tx bank send $SOURCE $DESTINATION "${AMOUNT}${DENOM}" --keyring-backend=test --chain-id=$NETWORK_NAME --fees "${FEE_AMOUNT}${FEE_DENOM}" --output=json --yes --home=$TSUKID_HOME | txAwait 180
...
# e.g. showStatus -> { ... }
function showStatus() {
    echo $(tsukid status 2>&1 | bash-utils jsonParse "" 2>/dev/null || echo -n "")
}
...
function activateValidator() {
    ...
    (! $(isNaturalNumber $TIMEOUT)) && TIMEOUT=180
    ($(isNullOrEmpty $ACCOUNT)) && echoInfo "INFO: Account name was not defined " && return 1
    tsukid tx customslashing activate --from "$ACCOUNT" --chain-id=$NETWORK_NAME --keyring-backend=test --home=$TSUKID_HOME --fees 1000ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
}
...
function pauseValidator() {
    ...
    (! $(isNaturalNumber $TIMEOUT)) && TIMEOUT=180
    ($(isNullOrEmpty $ACCOUNT)) && echoInfo "INFO: Account name was not defined " && return 1
    tsukid tx customslashing pause --from "$ACCOUNT" --chain-id=$NETWORK_NAME --keyring-backend=test --home=$TSUKID_HOME --fees 100ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
}
...
function unpauseValidator() {
    ...
    (! $(isNaturalNumber $TIMEOUT)) && TIMEOUT=180
    ($(isNullOrEmpty $ACCOUNT)) && echoInfo "INFO: Account name was not defined " && return 1
    tsukid tx customslashing unpause --from "$ACCOUNT" --chain-id=$NETWORK_NAME --keyring-backend=test --home=$TSUKID_HOME --fees 100ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
}

# clearPermission <account> <permission> <address> <timeout-seconds>
# e.g. clearPermission validator 11 tsukiXXX..YYY 180
function clearPermission() {
    ...
    if ($(isPermBlacklisted $ADDR $PERM)) ; then
        ...
        tsukid tx customgov permission remove-blacklisted --from "$ACCOUNT" --keyring-backend=test --permission="$PERM" --addr="$ADDR" --chain-id=$NETWORK_NAME --home=$TSUKID_HOME --fees=100ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
        ...
        tsukid tx customgov permission remove-whitelisted --from "$ACCOUNT" --keyring-backend=test --permission="$PERM" --addr="$ADDR" --chain-id=$NETWORK_NAME --home=$TSUKID_HOME --fees=100ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
    else
...
# e.g. whitelistPermission validator 11 tsukiXXX..YYY 180
function whitelistPermission() {
    ...
    if ($(isPermWhitelisted $ADDR $PERM)) ; then
        ...
        tsukid tx customgov permission whitelist --from "$KM_ACC" --keyring-backend=test --permission="$PERM" --addr="$ADDR" --chain-id=$NETWORK_NAME --home=$TSUKID_HOME --fees=100ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
    fi
}

# blacklisttPermission <account> <permission> <address> <timeout-seconds>
# e.g. blacklisttPermission validator 11 tsukiXXX..YYY 180
function blacklistPermission() {
    ...
    if ($(isPermBlacklisted $ADDR $PERM)) ; then
        ...
        tsukid tx customgov permission blacklist --from "$KM_ACC" --keyring-backend=test --permission="$PERM" --addr="$ADDR" --chain-id=$NETWORK_NAME --home=$TSUKID_HOME --fees=100ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
    fi
}

function showCurrentPlan() {
    tsukid query upgrade current-plan --output=json --chain-id=$NETWORK_NAME --home=$TSUKID_HOME
}

function showNextPlan() {
    tsukid query upgrade next-plan --output=json --chain-id=$NETWORK_NAME --home=$TSUKID_HOME
}
...
function showIdentityRecord() {
    ...
    if ($(isNullOrEmpty $KM_KEY)) ; then
        tsukid query customgov identity-records-by-addr $KM_ACC --output=json --home=$TSUKID_HOME | jq 2> /dev/null || echo ""
    else
        if ($(isNumber $KM_KEY)) ; then
            tsukid query customgov identity-records-by-addr $KM_ACC --output=json --home=$TSUKID_HOME | jq ".records | .[] | select(.id==\"$KM_KEY\")" 2> /dev/null || echo ""
        else
            tsukid query customgov identity-records-by-addr $KM_ACC --output=json --home=$TSUKID_HOME | jq ".records | .[] | select(.key==\"$KM_KEY\")" 2> /dev/null || echo ""
        fi
    fi
}
...
function upsertIdentityRecord() {
    ...
    if ($(isNullOrEmpty $IR_VAL)) ; then
        tsukid tx customgov delete-identity-records --keys="$IR_KEY" --from=$KM_ACC --keyring-backend=test --home=$TSUKID_HOME --chain-id=$NETWORK_NAME --fees=100ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
    else
        tsukid tx customgov register-identity-records --infos-json="{\"$IR_KEY\":\"$IR_VAL\"}" --from=$KM_ACC --keyring-backend=test --home=$TSUKID_HOME --chain-id=$NETWORK_NAME --fees=100ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
    fi
}
...
function verifyIdentityRecord() {
    ...
    tsukid tx customgov request-identity-record-verify --verifier="$KM_VER" --record-ids="$FINAL_IR_KEYS" --from=$KM_ACC --verifier-tip="$KM_TIP" --keyring-backend=test --home=$TSUKID_HOME --chain-id=$NETWORK_NAME --fees=100ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
}
...
function showIdentityVerificationRequests() {
    ...
    if ($(isNullOrEmpty $KM_REQ)) ; then
        tsukid query customgov identity-record-verify-requests-by-approver $KM_ACC --output=json --home=$TSUKID_HOME  | jq 2> /dev/null || echo ""
    else
        tsukid query customgov identity-record-verify-requests-by-approver $KM_ACC --output=json --home=$TSUKID_HOME  | jq ".verify_records | .[] | select(.address==\"$KM_REQ\")" 2> /dev/null || echo ""
    fi
}
...
function approveIdentityVerificationRequest() {
    ...
    tsukid tx customgov handle-identity-records-verify-request $KM_REQ --approve="true" --from=$ACCOUNT --keyring-backend=test --home=$TSUKID_HOME --chain-id=$NETWORK_NAME --fees=100ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
}
...
function rejectIdentityVerificationRequest() {
    ...
    tsukid tx customgov handle-identity-records-verify-request $KM_REQ --approve="false" --from=$ACCOUNT --keyring-backend=test --home=$TSUKID_HOME --chain-id=$NETWORK_NAME --fees=100ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
}
...
function showDataRegistryKeys() {
    tsukid query customgov data-registry-keys --page-key 1000000 --output=json | jq ".keys"
}

function showDataRegistryKey() {
    tsukid query customgov data-registry "$1" --output=json 2> /dev/null || echo ""
}
...
function setNetworkProperty() {
    ...
    tsukid tx customgov proposal set-network-property "${KEY^^}" "$VALUE" --title="Upserting Network Property '$KEY'" --description="Assign value '$VALUE' to property '$KEY'" --from=$ACCOUNT --chain-id=$NETWORK_NAME --keyring-backend=test  --fees=100ukex --yes --log_format=json --broadcast-mode=async --output=json --home=$TSUKID_HOME  | txAwait
}
...
function createRole() {
    ...
    (! $(isNaturalNumber $TIMEOUT)) && TIMEOUT=180
    tsukid tx customgov role create "$NAME" "$DESCRIPTION" --from "$ACCOUNT" --keyring-backend=test --chain-id=$NETWORK_NAME --home=$TSUKID_HOME --fees=100ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
}
...
function showRoles() {
    ...
    if ($(isNullOrEmpty $ADDRESS)) ; then
        echo $(tsukid query customgov all-roles --output=json --home=$TSUKID_HOME 2> /dev/null | jsonParse 2> /dev/null || echo -n "") && echo -n ""
    else
        echo $(tsukid query customgov roles $ADDRESS --output=json --home=$TSUKID_HOME 2> /dev/null | jsonParse 2> /dev/null || echo -n "") && echo -n ""
    fi
}

function showRole() {
    echo $(tsukid query customgov role "$1" --output=json --home=$TSUKID_HOME 2> /dev/null | jsonParse 2> /dev/null || echo -n "") && echo -n ""
}
...
function showProposalsDurations() {
    echo $(tsukid query customgov all-proposal-durations --output=json --home=$TSUKID_HOME 2> /dev/null | jsonParse 2> /dev/null || echo -n "") && echo -n ""
}

function showPoorNetworkMessages() {
    echo $(tsukid query customgov poor-network-messages --output=json --home=$TSUKID_HOME 2> /dev/null | jsonParse 2> /dev/null || echo -n "") && echo -n ""
}
...
function showExecutionFee() {
    ...
    local EXECUTION_FEE=$(tsukid query customgov execution-fee "$TRANSACTION_TYPE" --output=json --home=$TSUKID_HOME 2> /dev/null | jsonParse "fee.execution_fee" 2> /dev/null || echo -n "")
...
function setExecutionFee() {
    ...
    tsukid tx customgov set-execution-fee --transaction_type="$TX_TYPE" --execution_fee="$EXECUTION_FEE" --failure_fee="$FAILURE_FEE" --timeout="$TX_TIMEOUT" --from "$ACCOUNT" --chain-id=$NETWORK_NAME --keyring-backend=test  --fees=$TX_FEE --yes --log_format=json --broadcast-mode=async --output=json --home=$TSUKID_HOME | txAwait
}
...
function resetRanks() {
    ...
    tsukid tx customslashing proposal-reset-whole-validator-rank --title="Ranks reset" --description="Reseting ranks or all validator nodes" --from "$ACCOUNT" --chain-id=$NETWORK_NAME --keyring-backend=test  --fees=100ukex --yes --log_format=json --broadcast-mode=async --output=json --home=$TSUKID_HOME | txAwait
}

# showValidator <account/tsuki-address/val-address>
# e.g. showValidator validator
# e.g. showValidator tsukiXXXXXXXXXXX
# e.g. showValidator tsukivaloperXXXXXXXXXXX
function showValidator() {
    local VAL_ADDR="${1,,}"
    if [[ $VAL_ADDR == tsukivaloper* ]] ; then
        VAL_STATUS=$(tsukid query customstaking validator --val-addr="$VAL_ADDR" --output=json --home=$TSUKID_HOME 2> /dev/null | jsonParse 2> /dev/null || echo -n "")
    else
        local ADDRESS=$(showAddress $VAL_ADDR)
        VAL_STATUS=$(tsukid query customstaking validator --addr="$ADDRESS" --output=json --home=$TSUKID_HOME 2> /dev/null | jsonParse 2> /dev/null || echo -n "") 
    fi
    echo $VAL_STATUS
}

function showTokenAliases() {
    echo $(tsukid query tokens all-aliases --output=json --home=$TSUKID_HOME 2> /dev/null | jsonParse 2> /dev/null || echo -n "") 
}

function showTokenRates() {
    echo $(tsukid query tokens all-rates --output=json --home=$TSUKID_HOME 2> /dev/null | jsonParse 2> /dev/null || echo -n "") 
}
...
function showTokenTransferBlackWhiteList() {
    echo $(tsukid query tokens token-black-whites --output=json --home=$TSUKID_HOME 2> /dev/null | jsonParse 2> /dev/null || echo -n "") 
}
...
function showRolePermissions() {
    echo $(tsukid query customgov role "$1" --output=json --home=$TSUKID_HOME 2> /dev/null | jsonParse "permissions" 2> /dev/null || echo -n "") && echo -n ""
}
...
function roleClearPermission() {
    ...
    if ($(isRolePermBlacklisted "$ROLE" "$PERM")) ; then
        ...
        tsukid tx customgov role remove-blacklisted-permission "$ROLE" "$PERM" --from "$ACCOUNT" --keyring-backend=test --chain-id=$NETWORK_NAME --home=$TSUKID_HOME --fees=100ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
        ...
        tsukid tx customgov role remove-whitelisted-permission "$ROLE" "$PERM" --from "$ACCOUNT" --keyring-backend=test --chain-id=$NETWORK_NAME --home=$TSUKID_HOME --fees=100ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
    else
...
## e.g. roleWhitelistPermission validator validator 29 tsukiXXX..YYY 180
function roleWhitelistPermission() {
    ...
    if ($(isRolePermWhitelisted "$ROLE" "$PERM")) ; then
        ...
        tsukid tx customgov role whitelist-permission "$ROLE" "$PERM" --from "$ACCOUNT" --keyring-backend=test --chain-id=$NETWORK_NAME --home=$TSUKID_HOME --fees=100ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
    fi
}

## roleBlacklistPermission <account> <role> <permission> <timeout-seconds>
## e.g. roleBlacklistPermission validator validator 29 tsukiXXX..YYY 180
function roleBlacklistPermission() {
    ...
    if ($(isRolePermBlacklisted "$ROLE" "$PERM")) ; then
        ...
        tsukid tx customgov role blacklist-permission "$ROLE" "$PERM" --from "$ACCOUNT" --keyring-backend=test --chain-id=$NETWORK_NAME --home=$TSUKID_HOME --fees=100ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
    fi
}
...
function createRoleProposal() {
    ...
    ($(isNullOrEmpty $NAME)) && echoInfo "INFO: Invalid role name '$NAME' " && return 1
    (! $(isNaturalNumber $TIMEOUT)) && TIMEOUT=180
    tsukid tx customgov proposal create-role "$NAME" "$DESCRIPTION" --title="Upsert Governance Role '$NAME'" --description="Role description: '$DESCRIPTION'" --from "$ACCOUNT" --keyring-backend=test --chain-id=$NETWORK_NAME --home=$TSUKID_HOME --fees=100ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
}
...
function assignRole() {
    ...
    if ($(isRoleAssigned "$ROLE" "$ADDRESS")) ; then
        ...
        tsukid tx customgov role assign "$ROLE" --addr=$ADDRESS --from=$ACCOUNT --keyring-backend=test --chain-id=$NETWORK_NAME --fees=100ukex --yes --log_format=json --broadcast-mode=async --output=json --home=$TSUKID_HOME | txAwait $TIMEOUT
    fi
}
...
function unassignRole() {
    ...
    if (! $(isRoleAssigned "$ROLE" "$ADDRESS")) ; then
        ...
        tsukid tx customgov role remove "$ROLE" --addr=$ADDRESS --from=$ACCOUNT --keyring-backend=test --chain-id=$NETWORK_NAME --fees=100ukex --yes --log_format=json --broadcast-mode=async --output=json --home=$TSUKID_HOME | txAwait $TIMEOUT
    fi
}
...
function whitelistValidators() {
    ...
    if [ -f "$WHITELIST" ] ; then 
        ...
        while read key ; do
            ...
            tsukid tx bank send $ACCOUNT $key "954321ukex" --keyring-backend=test --chain-id=$NETWORK_NAME --fees 100ukex --yes --log_format=json --broadcast-mode=async --output=json --home=$TSUKID_HOME | txAwait $TIMEOUT
    ...
    elif ($(isTsukiAddress $WHITELIST)) ; then
        echoInfo "INFO: Fueling address $WHITELIST with funds from $ACCOUNT"
        tsukid tx bank send $ACCOUNT $WHITELIST "954321ukex" --keyring-backend=test --chain-id=$NETWORK_NAME --fees 100ukex --yes --log_format=json --broadcast-mode=async --output=json --home=$TSUKID_HOME | txAwait $TIMEOUT
...
# curl -s https://testnet-rpc.tsuki.network/api/valopers?all=true | jq '.validators | .[] | select(.status=="JAILED")'  | grep -o '".*"' | sed 's/"//g' | grep -o '\btsuki1\w*' | uniq > ./jailed
function unjailValidators() {
    ...
    elif ($(isTsukiAddress $ADDRESSES)) ; then
        echoInfo "INFO: Unjailing '$key' using account '$ACCOUNT'"
        unjail $ACCOUNT "$key" || echoErr "ERROR: Failed to unjail $key within ${TIMEOUT}s"
...
function addAccount() {
    ...
    tsukid keys add "$ACCOUNT" --keyring-backend=test --home=$TSUKID_HOME --output=json | jq
}
...
function recoverAccount() {
    ...
    ($(isAccount "$ACCOUNT")) && echoErr "ERROR: Account '$ACCOUNT' already exists" && return 1
    (! $(isAlphanumeric "$ACCOUNT")) && echoErr "ERROR: Account name must be alphanumeric, but got '$ACCOUNT'" && return 1
    yes "$MNEMONIC" | tsukid keys add $ACCOUNT --keyring-backend=test --home=$TSUKID_HOME --recover --output=json | jq
}
...
function deleteAccount() {
    ...
    if (! $(isAccount "$ACCOUNT")) ; then 
        ...
        tsukid keys delete "$ACCOUNT" --force --yes --keyring-backend=test --home=$TSUKID_HOME --output=json | jq
    fi
}
...
function enableCustody() {
  ...
  tsukid tx custody create $MODE $PASSWORD $LIMITS $WHITELIST --from=$FROM --keyring-backend=test --chain-id=$NETWORK_NAME --fees="${FEE_AMOUNT}${FEE_DENOM}" --output=json --yes --home=$TSUKID_HOME --okey=$OKEY --nkey=$NKEY --next=$NEXT --target=$TARGET | txAwait 180
}
# enableCustody
function disableCustody() {
  ...
  tsukid tx custody disable --from=$FROM --keyring-backend=test --chain-id=$NETWORK_NAME --fees="${FEE_AMOUNT}${FEE_DENOM}" --output=json --yes --home=$TSUKID_HOME --okey=$OKEY | txAwait 180
}
...
function addCustodians() {
  ...
  tsukid tx custody custodians add "$ADDRESSES" --from=$FROM --keyring-backend=test --chain-id=$NETWORK_NAME --fees="${FEE_AMOUNT}${FEE_DENOM}" --output=json --yes --home=$TSUKID_HOME --okey=$OKEY --nkey=$NKEY --next=$NEXT --target=$TARGET | txAwait 180
}
...
function addCustodiansForce() {
  ...
  tsukid tx custody custodians add "$ADDRESSES" --from=$FROM --keyring-backend=test --chain-id=$NETWORK_NAME --gas-prices="${FEE_AMOUNT}${FEE_DENOM}" --output=json --yes --home=$TSUKID_HOME --okey=$OKEY --nkey=$NKEY
}
...
function dropCustodians() {
  ...
  tsukid tx custody custodians drop --from=$FROM --keyring-backend=test --chain-id=$NETWORK_NAME --fees="${FEE_AMOUNT}${FEE_DENOM}" --output=json --yes --home=$TSUKID_HOME --okey=$OKEY --nkey=$NKEY | txAwait 180
}

# e.g. getCustodyInfo
function getCustodyKey() {
  local FROM=$(showAddress $1)
  local RESULT=$(tsukid query custody get $FROM --output=json --home=$TSUKID_HOME 2> /dev/null | jsonParse "custody_settings.key" 2> /dev/null || echo -n "")
...
function getCustodyInfo() {
  local FROM=$(showAddress $1)
  local RESULT=$(tsukid query custody get $FROM --output=json --home=$TSUKID_HOME 2> /dev/null | jsonParse "" 2> /dev/null || echo -n "")
...
function getCustodians() {
  local FROM=$(showAddress $1)
  local RESULT=$(tsukid query custody custodians get $FROM --output=json --home=$TSUKID_HOME 2> /dev/null | jsonParse "custody_custodians" 2> /dev/null || echo -n "")
...
function getCustodyPool() {
  local FROM=$(showAddress $1)
  local RESULT=$(tsukid query custody custodians pool $FROM --output=json --home=$TSUKID_HOME 2> /dev/null | jsonParse "transactions.record" 2> /dev/null || echo -n "")
...
function getCustodyPoolVotes() {
  ...
  local RESULT=$(tsukid query custody custodians pool $FROM --output=json --home=$TSUKID_HOME 2> /dev/null | jsonParse "transactions.record.$HASH.votes" 2> /dev/null || echo -n "")
...
function custodySendTokens() {
    ...
    RESULT=$(tsukid tx custody send $SOURCE $DESTINATION "${AMOUNT}${DENOM}" $PASSWORD --reward="${REWARD_AMOUNT}${REWARD_DENOM}" --keyring-backend=test --chain-id=$NETWORK_NAME --fees "${FEE_AMOUNT}${FEE_DENOM}" --output=json --yes --home=$TSUKID_HOME 2> /dev/null | txAwait2 180 2> /dev/null || echo -n "" )
...
function approveTransaction() {
    ...
    tsukid tx custody approve --from=$FROM $ADDRESS $HASH --keyring-backend=test --chain-id=$NETWORK_NAME --fees "${FEE_AMOUNT}${FEE_DENOM}" --output=json --yes --home=$TSUKID_HOME | txAwait 180
}
...
function declineTransaction() {
    ...
    tsukid tx custody decline --from=$FROM $ADDRESS $HASH --keyring-backend=test --chain-id=$NETWORK_NAME --fees "${FEE_AMOUNT}${FEE_DENOM}" --output=json --yes --home=$TSUKID_HOME | txAwait 180
}
