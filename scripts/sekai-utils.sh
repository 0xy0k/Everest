...
function tsukiUtilsSetup() {
    local TSUKI_UTILS_VERSION="v0.0.1.2"
    if [ "$1" == "version" ] ; then
        ...
        if [ "$VERSION" != "$TSUKI_UTILS_VERSION" ] ; then
            utils echoErr "ERROR: Self check version mismatch, expected '$TSUKI_UTILS_VERSION', but got '$VERSION'"
            return 1
            ...
            utils setGlobEnv TSUKI_TOOLS_SRC "$UTILS_DESTINATION"
            ...
            utils loadGlobEnvs
            utils echoInfo "INFO: SUCCESS, Installed tsuki-utils $(tsuki-utils tsukiUtilsVersion)"
        fi
    fi
}
...
function voteProposal() {
    ...
    tsukid tx customgov proposal vote $PROPOSAL $VOTE --from=$ACCOUNT --chain-id=$NETWORK_NAME --keyring-backend=test  --fees=100ukex --yes --log_format=json --broadcast-mode=async --output=json | txAwait
}
...
function sendTokens() {
    ...
    tsukid tx bank send $SOURCE $DESTINATION "${AMOUNT}${DENOM}" --keyring-backend=test --chain-id=$NETWORK_NAME --fees "${FEE_AMOUNT}${FEE_DENOM}" --output=json --yes | txAwait 180
...
# e.g. showStatus -> { ... }
function showStatus() {
    echo $(tsukid status 2>&1 | jsonParse "" 2>/dev/null || echo -n "")
}
...
function clearPermission() {
    ...
    if ($(isPermBlacklisted $ADDR $PERM)) ; then
        ...
        tsukid tx customgov permission remove-blacklisted-permission --from "$ACCOUNT" --keyring-backend=test --permission="$PERM" --addr="$ADDR" --chain-id=$NETWORK_NAME --home=$TSUKID_HOME --fees=100ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
        ...
        tsukid tx customgov permission remove-whitelisted-permission --from "$ACCOUNT" --keyring-backend=test --permission="$PERM" --addr="$ADDR" --chain-id=$NETWORK_NAME --home=$TSUKID_HOME --fees=100ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
    else
...
function whitelistPermission() {
    ...
    if ($(isPermWhitelisted $ADDR $PERM)) ; then
        ...
        tsukid tx customgov permission whitelist-permission --from "$KM_ACC" --keyring-backend=test --permission="$PERM" --addr="$ADDR" --chain-id=$NETWORK_NAME --home=$TSUKID_HOME --fees=100ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
    fi
}
...
function blacklistPermission() {
    ...
    if ($(isPermBlacklisted $ADDR $PERM)) ; then
        ...
        tsukid tx customgov permission blacklist-permission --from "$KM_ACC" --keyring-backend=test --permission="$PERM" --addr="$ADDR" --chain-id=$NETWORK_NAME --home=$TSUKID_HOME --fees=100ukex --yes --broadcast-mode=async --log_format=json --output=json | txAwait $TIMEOUT
    fi
}
...
function showIdentityRecord() {
    ...
    if ($(isNullOrEmpty $KM_KEY)) ; then
        tsukid query customgov identity-records-by-addr $KM_ACC --output=json | jq 2> /dev/null || echo ""
    else
        if ($(isNumber $KM_KEY)) ; then
            tsukid query customgov identity-records-by-addr $KM_ACC --output=json | jq ".records | .[] | select(.id==\"$KM_KEY\")" 2> /dev/null || echo ""
        else
            tsukid query customgov identity-records-by-addr $KM_ACC --output=json | jq ".records | .[] | select(.key==\"$KM_KEY\")" 2> /dev/null || echo ""
        fi
    fi
}
...
function showIdentityVerificationRequests() {
    ...
    if ($(isNullOrEmpty $KM_REQ)) ; then
        tsukid query customgov identity-record-verify-requests-by-approver $KM_ACC --output=json | jq 2> /dev/null || echo ""
    else
        tsukid query customgov identity-record-verify-requests-by-approver $KM_ACC --output=json | jq ".verify_records | .[] | select(.address==\"$KM_REQ\")" 2> /dev/null || echo ""
    fi
}
...
function upsertDataRegistry() {
    ...
    if [ "${DOWNLOAD_SUCCESS,,}" != "true" ] ; then
        ...
        tsukid tx customgov proposal upsert-data-registry "$KEY" "$CHECKSUM" "$VALUE" "$FILETYPE" "$SIZE" --title="Upserting Data Registry key '$KEY'" --description="Assign value '$VALUE' to key '$KEY'" --from=$ACCOUNT --chain-id=$NETWORK_NAME --keyring-backend=test  --fees=100ukex --yes --log_format=json --broadcast-mode=async --output=json | txAwait
    fi
}
...
function setNetworkProperty() {
    ...
    tsukid tx customgov proposal set-network-property "${KEY^^}" "$VALUE" --title="Upserting Network Property '$KEY'" --description="Assign value '$VALUE' to property '$KEY'" --from=$ACCOUNT --chain-id=$NETWORK_NAME --keyring-backend=test  --fees=100ukex --yes --log_format=json --broadcast-mode=async --output=json | txAwait
}
...
function setPoorNetworkMessages() {
    ...
    tsukid tx customgov proposal set-poor-network-msgs "$MESSAGES" --title="Update poor network messages" --description="Allowing submission of '[$MESSAGES]' during poor network conditions" --from "$ACCOUNT" --chain-id=$NETWORK_NAME --keyring-backend=test  --fees=100ukex --yes --log_format=json --broadcast-mode=async --output=json | txAwait
}
...
function setExecutionFee() {
    ...
    tsukid tx customgov set-execution-fee --transaction_type="$TX_TYPE" --execution_fee="$EXECUTION_FEE" --failure_fee="$FAILURE_FEE" --timeout="$TX_TIMEOUT" --from "$ACCOUNT" --chain-id=$NETWORK_NAME --keyring-backend=test  --fees=$TX_FEE --yes --log_format=json --broadcast-mode=async --output=json | txAwait
}
...
function resetRanks() {
    ...
    tsukid tx customslashing proposal-reset-whole-validator-rank --title="Ranks reset" --description="Reseting ranks or all validator nodes" --from "$ACCOUNT" --chain-id=$NETWORK_NAME --keyring-backend=test  --fees=100ukex --yes --log_format=json --broadcast-mode=async --output=json | txAwait
}
...
function assignRole() {
    ...
    if ($(isRoleAssigned "$ROLE" "$ADDRESS")) ; then
        ...
        tsukid tx customgov role assign "$ROLE" --addr=$ADDRESS --from=$ACCOUNT --keyring-backend=test --chain-id=$NETWORK_NAME --fees=100ukex --yes --log_format=json --broadcast-mode=async --output=json | txAwait $TIMEOUT
    fi
}
...
function removeRole() {
    ...
    if (! $(isRoleAssigned "$ROLE" "$ADDRESS")) ; then
        ...
        tsukid tx customgov role remove "$ROLE" --addr=$ADDRESS --from=$ACCOUNT --keyring-backend=test --chain-id=$NETWORK_NAME --fees=100ukex --yes --log_format=json --broadcast-mode=async --output=json | txAwait $TIMEOUT
    fi
}
...
function whitelistValidators() {
    ...
    if [ -f "$WHITELIST" ] ; then 
        ...
        while read key ; do
            ...
            tsukid tx bank send $ACCOUNT $key "954321ukex" --keyring-backend=test --chain-id=$NETWORK_NAME --fees 100ukex --yes --log_format=json --broadcast-mode=async --output=json | txAwait $TIMEOUT
        ...
        echoInfo "INFO: Fueling address $WHITELIST with funds from $ACCOUNT"
        tsukid tx bank send $ACCOUNT $WHITELIST "954321ukex" --keyring-backend=test --chain-id=$NETWORK_NAME --fees 100ukex --yes --log_format=json --broadcast-mode=async --output=json | txAwait $TIMEOUT
