...
tsukid tx basket burn-basket-tokens 1 1000b1/usd --from=validator --keyring-backend=test --home=$HOME/.tsukid --chain-id=testing --fees=100ukex --yes --broadcast-mode=block
...
tsukid tx basket proposal-basket-withdraw-surplus 1,2 $(tsukid keys show -a validator --keyring-backend=test) --title="title" --description="description" --from=validator --keyring-backend=test --home=$HOME/.tsukid --chain-id=testing --fees=100ukex --yes --broadcast-mode=block
