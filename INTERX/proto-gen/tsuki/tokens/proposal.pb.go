// Code generated by protoc-gen-go. DO NOT EDIT.
// source: tsuki/tokens/proposal.proto

package proto

import (
	fmt "fmt"
	_ "github.com/gogo/protobuf/gogoproto"
	proto "github.com/golang/protobuf/proto"
	_ "github.com/regen-network/cosmos-proto"
	math "math"
)

// Reference imports to suppress errors if they are not otherwise used.
var _ = proto.Marshal
var _ = fmt.Errorf
var _ = math.Inf

// This is a compile-time assertion to ensure that this generated file
// is compatible with the proto package it is being compiled against.
// A compilation error at this line likely means your copy of the
// proto package needs to be updated.
const _ = proto.ProtoPackageIsVersion3 // please upgrade the proto package

type MsgProposalUpsertTokenAlias struct {
	Description          string   `protobuf:"bytes,7,opt,name=description,proto3" json:"description,omitempty"`
	Symbol               string   `protobuf:"bytes,1,opt,name=symbol,proto3" json:"symbol,omitempty"`
	Name                 string   `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	Icon                 string   `protobuf:"bytes,3,opt,name=icon,proto3" json:"icon,omitempty"`
	Decimals             uint32   `protobuf:"varint,4,opt,name=decimals,proto3" json:"decimals,omitempty"`
	Denoms               []string `protobuf:"bytes,5,rep,name=denoms,proto3" json:"denoms,omitempty"`
	Proposer             []byte   `protobuf:"bytes,6,opt,name=proposer,proto3" json:"proposer,omitempty"`
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}

func (m *MsgProposalUpsertTokenAlias) Reset()         { *m = MsgProposalUpsertTokenAlias{} }
func (m *MsgProposalUpsertTokenAlias) String() string { return proto.CompactTextString(m) }
func (*MsgProposalUpsertTokenAlias) ProtoMessage()    {}
func (*MsgProposalUpsertTokenAlias) Descriptor() ([]byte, []int) {
	return fileDescriptor_68008d794328e180, []int{0}
}

func (m *MsgProposalUpsertTokenAlias) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_MsgProposalUpsertTokenAlias.Unmarshal(m, b)
}
func (m *MsgProposalUpsertTokenAlias) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_MsgProposalUpsertTokenAlias.Marshal(b, m, deterministic)
}
func (m *MsgProposalUpsertTokenAlias) XXX_Merge(src proto.Message) {
	xxx_messageInfo_MsgProposalUpsertTokenAlias.Merge(m, src)
}
func (m *MsgProposalUpsertTokenAlias) XXX_Size() int {
	return xxx_messageInfo_MsgProposalUpsertTokenAlias.Size(m)
}
func (m *MsgProposalUpsertTokenAlias) XXX_DiscardUnknown() {
	xxx_messageInfo_MsgProposalUpsertTokenAlias.DiscardUnknown(m)
}

var xxx_messageInfo_MsgProposalUpsertTokenAlias proto.InternalMessageInfo

func (m *MsgProposalUpsertTokenAlias) GetDescription() string {
	if m != nil {
		return m.Description
	}
	return ""
}

func (m *MsgProposalUpsertTokenAlias) GetSymbol() string {
	if m != nil {
		return m.Symbol
	}
	return ""
}

func (m *MsgProposalUpsertTokenAlias) GetName() string {
	if m != nil {
		return m.Name
	}
	return ""
}

func (m *MsgProposalUpsertTokenAlias) GetIcon() string {
	if m != nil {
		return m.Icon
	}
	return ""
}

func (m *MsgProposalUpsertTokenAlias) GetDecimals() uint32 {
	if m != nil {
		return m.Decimals
	}
	return 0
}

func (m *MsgProposalUpsertTokenAlias) GetDenoms() []string {
	if m != nil {
		return m.Denoms
	}
	return nil
}

func (m *MsgProposalUpsertTokenAlias) GetProposer() []byte {
	if m != nil {
		return m.Proposer
	}
	return nil
}

type ProposalUpsertTokenAlias struct {
	Symbol               string   `protobuf:"bytes,1,opt,name=symbol,proto3" json:"symbol,omitempty"`
	Name                 string   `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	Icon                 string   `protobuf:"bytes,3,opt,name=icon,proto3" json:"icon,omitempty"`
	Decimals             uint32   `protobuf:"varint,4,opt,name=decimals,proto3" json:"decimals,omitempty"`
	Denoms               []string `protobuf:"bytes,5,rep,name=denoms,proto3" json:"denoms,omitempty"`
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}

func (m *ProposalUpsertTokenAlias) Reset()         { *m = ProposalUpsertTokenAlias{} }
func (m *ProposalUpsertTokenAlias) String() string { return proto.CompactTextString(m) }
func (*ProposalUpsertTokenAlias) ProtoMessage()    {}
func (*ProposalUpsertTokenAlias) Descriptor() ([]byte, []int) {
	return fileDescriptor_68008d794328e180, []int{1}
}

func (m *ProposalUpsertTokenAlias) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_ProposalUpsertTokenAlias.Unmarshal(m, b)
}
func (m *ProposalUpsertTokenAlias) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_ProposalUpsertTokenAlias.Marshal(b, m, deterministic)
}
func (m *ProposalUpsertTokenAlias) XXX_Merge(src proto.Message) {
	xxx_messageInfo_ProposalUpsertTokenAlias.Merge(m, src)
}
func (m *ProposalUpsertTokenAlias) XXX_Size() int {
	return xxx_messageInfo_ProposalUpsertTokenAlias.Size(m)
}
func (m *ProposalUpsertTokenAlias) XXX_DiscardUnknown() {
	xxx_messageInfo_ProposalUpsertTokenAlias.DiscardUnknown(m)
}

var xxx_messageInfo_ProposalUpsertTokenAlias proto.InternalMessageInfo

func (m *ProposalUpsertTokenAlias) GetSymbol() string {
	if m != nil {
		return m.Symbol
	}
	return ""
}

func (m *ProposalUpsertTokenAlias) GetName() string {
	if m != nil {
		return m.Name
	}
	return ""
}

func (m *ProposalUpsertTokenAlias) GetIcon() string {
	if m != nil {
		return m.Icon
	}
	return ""
}

func (m *ProposalUpsertTokenAlias) GetDecimals() uint32 {
	if m != nil {
		return m.Decimals
	}
	return 0
}

func (m *ProposalUpsertTokenAlias) GetDenoms() []string {
	if m != nil {
		return m.Denoms
	}
	return nil
}

type MsgProposalUpsertTokenRates struct {
	Description          string   `protobuf:"bytes,5,opt,name=description,proto3" json:"description,omitempty"`
	Denom                string   `protobuf:"bytes,1,opt,name=denom,proto3" json:"denom,omitempty"`
	Rate                 string   `protobuf:"bytes,2,opt,name=rate,proto3" json:"rate,omitempty"`
	FeePayments          bool     `protobuf:"varint,3,opt,name=fee_payments,json=feePayments,proto3" json:"fee_payments,omitempty"`
	Proposer             []byte   `protobuf:"bytes,4,opt,name=proposer,proto3" json:"proposer,omitempty"`
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}

func (m *MsgProposalUpsertTokenRates) Reset()         { *m = MsgProposalUpsertTokenRates{} }
func (m *MsgProposalUpsertTokenRates) String() string { return proto.CompactTextString(m) }
func (*MsgProposalUpsertTokenRates) ProtoMessage()    {}
func (*MsgProposalUpsertTokenRates) Descriptor() ([]byte, []int) {
	return fileDescriptor_68008d794328e180, []int{2}
}

func (m *MsgProposalUpsertTokenRates) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_MsgProposalUpsertTokenRates.Unmarshal(m, b)
}
func (m *MsgProposalUpsertTokenRates) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_MsgProposalUpsertTokenRates.Marshal(b, m, deterministic)
}
func (m *MsgProposalUpsertTokenRates) XXX_Merge(src proto.Message) {
	xxx_messageInfo_MsgProposalUpsertTokenRates.Merge(m, src)
}
func (m *MsgProposalUpsertTokenRates) XXX_Size() int {
	return xxx_messageInfo_MsgProposalUpsertTokenRates.Size(m)
}
func (m *MsgProposalUpsertTokenRates) XXX_DiscardUnknown() {
	xxx_messageInfo_MsgProposalUpsertTokenRates.DiscardUnknown(m)
}

var xxx_messageInfo_MsgProposalUpsertTokenRates proto.InternalMessageInfo

func (m *MsgProposalUpsertTokenRates) GetDescription() string {
	if m != nil {
		return m.Description
	}
	return ""
}

func (m *MsgProposalUpsertTokenRates) GetDenom() string {
	if m != nil {
		return m.Denom
	}
	return ""
}

func (m *MsgProposalUpsertTokenRates) GetRate() string {
	if m != nil {
		return m.Rate
	}
	return ""
}

func (m *MsgProposalUpsertTokenRates) GetFeePayments() bool {
	if m != nil {
		return m.FeePayments
	}
	return false
}

func (m *MsgProposalUpsertTokenRates) GetProposer() []byte {
	if m != nil {
		return m.Proposer
	}
	return nil
}

type ProposalUpsertTokenRates struct {
	Denom                string   `protobuf:"bytes,1,opt,name=denom,proto3" json:"denom,omitempty"`
	Rate                 string   `protobuf:"bytes,2,opt,name=rate,proto3" json:"rate,omitempty"`
	FeePayments          bool     `protobuf:"varint,3,opt,name=fee_payments,json=feePayments,proto3" json:"fee_payments,omitempty"`
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}

func (m *ProposalUpsertTokenRates) Reset()         { *m = ProposalUpsertTokenRates{} }
func (m *ProposalUpsertTokenRates) String() string { return proto.CompactTextString(m) }
func (*ProposalUpsertTokenRates) ProtoMessage()    {}
func (*ProposalUpsertTokenRates) Descriptor() ([]byte, []int) {
	return fileDescriptor_68008d794328e180, []int{3}
}

func (m *ProposalUpsertTokenRates) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_ProposalUpsertTokenRates.Unmarshal(m, b)
}
func (m *ProposalUpsertTokenRates) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_ProposalUpsertTokenRates.Marshal(b, m, deterministic)
}
func (m *ProposalUpsertTokenRates) XXX_Merge(src proto.Message) {
	xxx_messageInfo_ProposalUpsertTokenRates.Merge(m, src)
}
func (m *ProposalUpsertTokenRates) XXX_Size() int {
	return xxx_messageInfo_ProposalUpsertTokenRates.Size(m)
}
func (m *ProposalUpsertTokenRates) XXX_DiscardUnknown() {
	xxx_messageInfo_ProposalUpsertTokenRates.DiscardUnknown(m)
}

var xxx_messageInfo_ProposalUpsertTokenRates proto.InternalMessageInfo

func (m *ProposalUpsertTokenRates) GetDenom() string {
	if m != nil {
		return m.Denom
	}
	return ""
}

func (m *ProposalUpsertTokenRates) GetRate() string {
	if m != nil {
		return m.Rate
	}
	return ""
}

func (m *ProposalUpsertTokenRates) GetFeePayments() bool {
	if m != nil {
		return m.FeePayments
	}
	return false
}

func init() {
	proto.RegisterType((*MsgProposalUpsertTokenAlias)(nil), "tsuki.tokens.MsgProposalUpsertTokenAlias")
	proto.RegisterType((*ProposalUpsertTokenAlias)(nil), "tsuki.tokens.ProposalUpsertTokenAlias")
	proto.RegisterType((*MsgProposalUpsertTokenRates)(nil), "tsuki.tokens.MsgProposalUpsertTokenRates")
	proto.RegisterType((*ProposalUpsertTokenRates)(nil), "tsuki.tokens.ProposalUpsertTokenRates")
}

func init() {
	proto.RegisterFile("tsuki/tokens/proposal.proto", fileDescriptor_68008d794328e180)
}

var fileDescriptor_68008d794328e180 = []byte{
	// 441 bytes of a gzipped FileDescriptorProto
	,
}
