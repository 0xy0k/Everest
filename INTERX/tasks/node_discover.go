...
func QueryTsukiStatus(rpcAddr string) (tmTypes.ResultStatus, error) {
...
func QueryStatus(ipAddr string) (tmTypes.ResultStatus, error) {
	result, err := QueryTsukiStatus("http://" + ipAddr + ":16657")
	if err == nil {
		return result, err
	}

	result, err = QueryTsukiStatus("http://" + ipAddr + ":26657")
	if err == nil {
		return result, err
	}

	result, err = QueryTsukiStatus("http://" + ipAddr + ":36657")
	if err == nil {
		return result, err
	}

	result, err = QueryTsukiStatus("http://" + ipAddr + ":46657")
	if err == nil {
		return result, err
	}

	result, err = QueryTsukiStatus("http://" + ipAddr + ":56657")
	if err == nil {
		return result, err
	}
...
func NodeDiscover(rpcAddr string, isLog bool) {
	...
	for {
		...
		for index < len(uniqueIPAddresses) {
			...
			tsukiStatus, err := QueryStatus(ipAddr)
			if err != nil {
				continue
			}

			synced := false
			blockDiff := common.NodeStatus.Block - tsukiStatus.SyncInfo.LatestBlockHeight
			if blockDiff >= -BLOCK_DIFF_LIMIT && blockDiff <= BLOCK_DIFF_LIMIT {
				synced = true
			}

			nodeInfo := types.P2PNode{}
			nodeInfo.ID = string(tsukiStatus.NodeInfo.ID())
			nodeInfo.IP = ipAddr
			nodeInfo.Port, _ = getPort(tsukiStatus.NodeInfo.ListenAddr)
