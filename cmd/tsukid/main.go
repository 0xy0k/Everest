...
func appExport(
	...
	var tsukiApp *app.TsukiApp
	if height != -1 {
		tsukiApp = app.NewInitApp(logger, db, traceStore, false, map[int64]bool{}, "", uint(1), encCfg, appOpts)

		if err := tsukiApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		tsukiApp = app.NewInitApp(logger, db, traceStore, true, map[int64]bool{}, "", uint(1), encCfg, appOpts)
	}

	return tsukiApp.ExportAppStateAndValidators(forZeroHeight, jailAllowedAddrs)
}
