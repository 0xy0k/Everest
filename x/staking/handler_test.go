	...
	tests := []struct {
		...
		},
		{
			...
			func(ctx types.Context, app *simapp.SimApp) {
				...
				valAddr2, err := types.ValAddressFromBech32("tsukivaloper15ky9du8a2wlstz6fpx3p4mqpryrm5cgqeyf3v0")
				require.NoError(t, err)
...
func TestHandler_ProposalUnjailValidator_Errors(t *testing.T) {
	proposerAddr, err := types.AccAddressFromBech32("tsuki1alzyfq40zjsveat87jlg8jxetwqmr0a29sgd0f")
	require.NoError(t, err)
