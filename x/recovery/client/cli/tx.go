...
$ <appd> tx recovery rotate-recovery-address [recovery] [proof] --from validator --chain-id=testing --keyring-backend=test --fees=100ukex --home=$HOME/.tsukid --yes
`,
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
...
$ <appd> tx recovery rotate-validator-by-half-rr-holder [address] [recovery] --from validator --chain-id=testing --keyring-backend=test --fees=100ukex --home=$HOME/.tsukid --yes
`,
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
