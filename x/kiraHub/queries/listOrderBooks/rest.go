...
func GetOrderBooks(cliCtx client.Context) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		res, _, err := cliCtx.QueryWithData(fmt.Sprintf("custom/tsukiHub/listOrderBooks/%s/%s", by, value), nil)
		if err != nil {
...
func GetOrderBooksByTP(cliCtx client.Context) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		res, _, err := cliCtx.QueryWithData(fmt.Sprintf("custom/tsukiHub/listOrderBooks/tp/%s/%s", base, quote), nil)
		if err != nil {
			rest.WriteErrorResponse(w, http.StatusNotFound, err.Error())
			return
		}
