...
func TestKeeper_GetNetworkActorsByAbsoluteWhitelistPermission(t *testing.T) {
	tests := []struct {
		name       string
		prepareApp func(app *simapp.TsukiApp, ctx sdk.Context) []types.NetworkActor
	}{
		{
...
func addRoleToMultipleAddrs(app *simapp.TsukiApp, ctx sdk.Context, addrs []sdk.AccAddress, role types.Role) {
	for _, addr := range addrs {
		app.CustomGovKeeper.AssignRoleToActor(ctx, types.NewDefaultActor(addr), role)
	}
}

func whitelistPermToMultipleAddrs(app *simapp.TsukiApp, ctx sdk.Context, addrs []sdk.AccAddress, permissions types.PermValue) error {
	for _, addr := range addrs {
