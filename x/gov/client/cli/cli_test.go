...
import (
	...
	types2 "github.com/TsukiCore/tsuki/x/gov/types"
...
func (s IntegrationTestSuite) TestGetTxSetWhitelistPermissions() {
	...
	addr, err := types3.AccAddressFromBech32("tsuki15ky9du8a2wlstz6fpx3p4mqpjyrm5cgqzp4f3d")
	s.Require().NoError(err)
...
func (s IntegrationTestSuite) TestGetTxSetBlacklistPermissions() {
	...
	// We create some random address where we will give perms.
	addr, err := types3.AccAddressFromBech32("tsuki1alzyfq40zjsveat87jlg8jxetwqmr0a29sgd0f")
	s.Require().NoError(err)
